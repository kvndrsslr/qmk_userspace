...
#define IS_MOD_TAP_CAG(k)   (IS_QK_MOD_TAP(k) && (k) & (QK_LCTL|QK_LALT|QK_LGUI))
    ...
    #define IS_HOMEROW(r)          (r->event.key.col == L_HOMEROW || r->event.key.col == R_HOMEROW)

    #define IS_UNILATERAL(r, n) ( \
        (r->event.key.col == L_HOMEROW && 0 <= n.event.key.col && n.event.key.col <= 6) || \
        (r->event.key.col == R_HOMEROW && 7 <= n.event.key.col && n.event.key.col <= 13) )

    #define IS_BILATERAL(r, n) ( \
        (r->event.key.col == L_HOMEROW && 7 <= n.event.key.col && n.event.key.col <= 13) || \
        (r->event.key.col == R_HOMEROW && 0 <= n.event.key.col && n.event.key.col <= 6) )

#else

    #define IS_HOMEROW(r)          (r->event.key.row == L_HOMEROW || r->event.key.row == R_HOMEROW)

    #define IS_UNILATERAL(r, n) ( \
        (r->event.key.row == L_HOMEROW && 0 <= n.event.key.row && n.event.key.row <= 6) || \
        (r->event.key.row == R_HOMEROW && 7 <= n.event.key.row && n.event.key.row <= 13) )

    #define IS_BILATERAL(r, n) ( \
        (r->event.key.row == L_HOMEROW && 7 <= n.event.key.row && n.event.key.row <= 13) || \
        (r->event.key.row == R_HOMEROW && 0 <= n.event.key.row && n.event.key.row <= 6) )
...
bool pre_process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed) {
        ...
        next_record  = *record;
        // // Press the tap keycode if the tap-hold key follows the previous key rapidly
        // if ((IS_HOMEROW_CAG(keycode, record) || IS_SHORTCUT(keycode)) && IS_TYPING_ALT(prev_keycode)) {
...
uint16_t get_tapping_term(uint16_t keycode, keyrecord_t *record) {
    // Fine tune values
    if (IS_HOMEROW_SHIFT(keycode, record)) return TAPPING_TERM - 75;
